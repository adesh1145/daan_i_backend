...
class NGODetailModel(models.Model):
    ...
    ngo_image = models.ImageField(blank=True,)  # Store image URL
    ...
    accept_category = models.ManyToManyField(CategoryModel, )
    ...
    state = models.ForeignKey(
        StateModel, related_name='users', on_delete=models.CASCADE, blank=True, null=True)
    ...
    def clean(self):
        ...
        if cleaned_data.get('state') and cleaned_data.get('city'):
            ...
            if not CityModel.objects.filter(
                    id=cleaned_data.get('city').id, state=cleaned_data.get('state')).exists():

                raise forms.ValidationError(message=f"{cleaned_data.get(
                    'city').name} is Not Exist in {cleaned_data.get('state').name}")
        return cleaned_data
