...
def custom_exception_handler(exc, context):
    ...
    if response is not None and 'token_not_valid' in str(exc):
        ...
        return responseModel(
            ...
            status=False, statusCode=status.HTTP_401_UNAUTHORIZED)
    if response is not None and 'authentication_failed' in str(exc) and '404' in str(exc):

        return responseModel(
            msg="Data NOt Found",
            status=False, statusCode=status.HTTP_404_NOT_FOUND)
    elif response is not None and 'authentication_failed' in str(exc) and '400' in str(exc):

        return responseModel(
            msg="Token Is Required",
            status=False, statusCode=status.HTTP_400_BAD_REQUEST)
